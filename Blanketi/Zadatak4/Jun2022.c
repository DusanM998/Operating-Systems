/*Linux program koji u zadatom direktorijumu rekurzivno(maksimalno do dubine 3) brise sve regularne datoteke koje u imenu
 * sadrze podstring "log" i vece su od 15KB. Za brisanje datoteka koristiti rm komandu Linux operativnog sistema.*/
...
void processdir(char *path, int level)
{
    ...
    while((dirp = readdir(dp)) != NULL)
    {
        ...
        if(S_ISREG(statbuf.st_mode) && statbuf.st_size > 15 * 1024 && strstr(dirp->d_name, "log") != NULL)
        {
