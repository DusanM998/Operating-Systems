/*Linux program koji se deli na dva procesa. Proces roditelj sa standardnog ulaza cita liniju po liniju teksta
 * i koriscenjem deljene memorije procitane linije teksta prosledjuje procesu detetu. Proces dete stampa na
 * standardnom izlazu linije teksta koje prihvata iz deljene memorije. Obezbediti da proces dete stampa linije
 * teksta onim redosledom kako se javljaju u tekstualnoj datoteci. Ovaj proces se ponavlja sve dok korisnik
 * na standardnom izlazu ne unese rec "KRAJ".*/
...
int main()
{
    ...
    if(fork() != 0)
    {
        //Proces roditelj: cita liniju po liniju teksta i koriscenjem
    ...
    {
        //Proces dete stampa linije teksta koje prihvati iz deljene memorije
        char line[80];
        int shm_id = shmget(MEM_KEY, 80*sizeof(char), IPC_CREAT | 0666);
